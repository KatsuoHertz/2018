...
# 2 次元配列データを画像データ化。
# ignore_flag: 指定した画素値の画素は表示しない（０にする）オプション
# ignore_vals: ここで指定した画素値と一致する画素は画素値を０にする。
def make_image( dat, ignore_flag = False, ignore_vals = None ):
    color_table = [
        [  0,   0, 255],
        [  0, 255,   0],
        [255,   0,   0],
    ...
    if dat.dtype == np.int32 or dat.dtype == np.int64:
        ...
        img[dat == 0] = [0, 0, 0]
        if ignore_flag and ignore_vals:
...
if __name__ == "__main__":
    ...
    parser.add_argument("-i", "--ignore_vals", type=int, nargs="+", help="画素値が指定した値の画素は表示しない（黒にする）")
    args = parser.parse_args()

    # 引数処理
    zoom = int(args.zoom)
    ignore_vals = args.ignore_vals
    ...
    while(f < len(npz)):
        ...
        for c in range(len(npz[f])):
            ...
            # 画像化
            img = make_image(dat, ignore_flag, ignore_vals)
